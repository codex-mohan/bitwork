...
import { SavedJobsView } from "@/components/dashboard/saved-jobs-view";

export const metadata = {
  title: "Saved Jobs | Bitwork",
...
export default async function SavedJobsPage() {
  ...
  // Fetch saved jobs on the server
  const savedJobs = await getSavedJobs(user.id);

  return (
    <DashboardLayout user={userData}>
      <SavedJobsView initialJobs={savedJobs} userId={user.id} />
    </DashboardLayout>
  );
}
